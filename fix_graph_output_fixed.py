...
def detect_and_fix_matplotlib():
    ...
    for backend in backends_to_try:
        try:
            ...
            if backend == 'Agg':
                ...
                print(f"[OK] {backend} backend works (saves to file)")
                if os.path.exists('test_plot.png'):
                    os.remove('test_plot.png')  # Clean up
            else:
                # For interactive backends, try to show
                plt.show(block=False)
                print(f"[OK] {backend} backend works (interactive)")
    ...
    if working_backend:
        print(f"\n[OK] Found working backend: {working_backend}")
        return working_backend
    else:
        print("\n[FAIL] No working backend found!")
        return None
...
def test_plotting():
    ...
    print("[OK] All plotting tests completed!")
...
"""
    ...
    with open('improved_plotting.py', 'w', encoding='utf-8') as f:
        f.write(code)
    
    print("[OK] Created improved_plotting.py with enhanced plotting functions")
...
def main():
    ...
    try:
        ...
        config_content = f"""# Matplotlib configuration for quantitative analysis
...
"""
        ...
        print(f"[OK] Created matplotlib config at: {config_file}")
    ...
    if working_backend:
        print(f"[OK] Working matplotlib backend: {working_backend}")
    else:
        print("[WARNING] No interactive backend found - plots will be saved as files")
    
    print("[OK] Created improved_plotting.py with enhanced functions")
    print("[OK] Created matplotlib configuration")
