...
class QuantitativeAnalysis:
    ...
    def load_stock_data(self, tickers, start_date='2020-01-01', end_date='2023-01-01', max_retries=3):
        ...
        try:
            ...
            bulk_data = yf.download(valid_tickers, start=start_date, end=end_date, progress=False, group_by='ticker', auto_adjust=True)
            
            if bulk_data is not None and not bulk_data.empty:
                # Handle single vs multiple tickers
                if len(valid_tickers) == 1:
                    # Single ticker - extract Close prices
                    if 'Close' in bulk_data.columns:
                        self.data = pd.DataFrame({valid_tickers[0]: bulk_data['Close']})
                    else:
                        # Handle case where columns might be different
                        self.data = pd.DataFrame({valid_tickers[0]: bulk_data.iloc[:, 0]})
                    
                    # Remove NaN values
                    self.data = self.data.dropna()
                    
                    if len(self.data) >= 10:
                        print(f"✓ Successfully loaded {valid_tickers[0]} ({len(self.data)} data points)")
                    ...
                    for ticker in valid_tickers:
                        try:
                            ...
                            elif ticker in bulk_data.columns.get_level_values(1):
                                # Handle case where ticker is in second level
    ...
    def monte_carlo_simulation(self, num_simulations=1000, time_horizon=252):
        ...
        try:
            ...
            results = np.zeros((num_simulations, 3))  # Always 3 columns: Return, Volatility, Sharpe Ratio
            ...
            mc_results = pd.DataFrame(results, columns=['Return', 'Volatility', 'Sharpe Ratio'])
